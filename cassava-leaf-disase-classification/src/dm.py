...
class DataModule(pl.LightningDataModule):
    ...
    def __init__(self, path = 'data', file = 'train_processed.csv', size=256, batch_size = 64, test_size = 0.2, seed = 42, subset=False, **kwargs):
        super().__init__()
        self.path = path
        self.file = file
    ...
    def setup(self, stage=None):
        ...
        self.train_ds = Dataset(self.path, train['image_id'].values, train['label'].values, train_trans)
        # val dataset
        val_trans = torchvision.transforms.CenterCrop(self.size)
        self.val_ds = Dataset(self.path, val['image_id'].values, val['label'].values, val_trans)
